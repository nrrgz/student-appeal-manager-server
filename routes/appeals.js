...
// @route   PUT /api/appeals/:id/withdraw
// @desc    Withdraw appeal (student's own appeal)
// @access  Private (Student)
router.put(
  "/:id/withdraw",
  [auth, requireStudent, body("reason").optional().trim()],
  async (req, res) => {
    try {
      const appeal = await Appeal.findOne({
        _id: req.params.id,
      ...
      if (appeal.status === "resolved" || appeal.status === "rejected") {
        return res.status(400).json({
          message:
            "Cannot withdraw appeal that is already resolved or rejected",
        });
      }
      ...
      appeal.timeline.push({
        ...
        description: `Appeal withdrawn by student: ${req.user.firstName} ${
          req.user.lastName
      ...
      res
        .status(500)
        .json({ message: "Server error while withdrawing appeal" });
    }
  }
);

// @route   GET /api/appeals/:id/timeline
// @desc    Get appeal timeline (student's own appeal)
// @access  Private (Student)
router.get("/:id/timeline", auth, requireStudent, async (req, res) => {
  try {
    const appeal = await Appeal.findOne({
      _id: req.params.id,
      student: req.user._id,
    ...
    res.json({ timeline: appeal.timeline });
  } catch (error) {
...
// @route   GET /api/appeals/:id/notes
// @desc    Get appeal notes (student's own appeal)
// @access  Private (Student)
router.get("/:id/notes", auth, requireStudent, async (req, res) => {
  try {
    const appeal = await Appeal.findOne({
      _id: req.params.id,
      student: req.user._id,
    ...
    // Students can only see public notes
    const publicNotes = appeal.notes.filter((note) => !note.isInternal);
